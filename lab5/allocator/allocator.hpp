...
namespace allocators {
    ...
    class Allocator {
    private:
        std::list<T*> _used_blocks; // исходные ячейки с данными
        T **_free_blocks; // указатели на оставшиеся свободные ячейки
        uint64_t _free_count; // количество свободных ячеек
    public:
        using value_type = T;
        using pointer = T *;
        ...
        Allocator() {
            _used_blocks.push_back(static_cast<T*>(malloc(sizeof(T) * BLOCK_SIZE)));
